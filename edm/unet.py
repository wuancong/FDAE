...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.additional_cond_map_layer_dim = additional_cond_map_layer_dim
        ...
        if self.additional_cond_map_layer_dim > 0:
            self.input_blocks = nn.ModuleList(
                [
                    TimestepEmbedSequential(conv_nd(dims, in_channels, self.additional_cond_map_layer_dim, 3, padding=1)),
                    TimestepEmbedSequential(conv_nd(dims, self.additional_cond_map_layer_dim, ch, 3, padding=1))
                ]
            )
        else:
            self.input_blocks = nn.ModuleList(
    ...
    def forward(self, x, timesteps, y=None, condition_map=None, condition_idx=None):
        """
        ...
        :param condition_map: a list of conditional map for adding to hs
        :param condition_idx: a list of indices for selecting feature maps in hs (len(condition_map)=len(condition_idx))
        :return: an [N x C x ...] Tensor of outputs.
        """
        ...
        h = x.type(self.dtype)
        for hs_idx, module in enumerate(self.input_blocks):
            h = module(h, emb)
            if condition_idx is not None and hs_idx in condition_idx:
                if condition_map is not None:
                    cmap = condition_map[condition_idx.index(hs_idx)].type(self.dtype)
                    h = h + cmap
